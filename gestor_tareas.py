...
def gestor_tareas(parent):
    ...
    entrada = tk.Entry(
        ventana, font=FUENTE_TEXTO,
        bg=COLOR_LISTA_FONDO, fg=COLOR_TEXTO_GENERAL, insertbackground=COLOR_TEXTO_GENERAL
    )
    ...
    tk.Button(
        btn_frame, text="Agregar", command=agregar,
        bg=COLOR_BOTON, fg=COLOR_BOTON_TEXTO, font=FUENTE_TEXTO
    ).pack(side=tk.LEFT, padx=5)

    tk.Button(
        btn_frame, text="Eliminar", command=eliminar,
        bg=COLOR_BOTON, fg=COLOR_BOTON_TEXTO, font=FUENTE_TEXTO
    ).pack(side=tk.LEFT, padx=5)

    tk.Button(
        btn_frame, text="Completar", command=completar,
        bg=COLOR_BOTON, fg=COLOR_BOTON_TEXTO, font=FUENTE_TEXTO
    ).pack(side=tk.LEFT, padx=5)

    lista = tk.Listbox(
        ventana, font=FUENTE_TEXTO,
        bg=COLOR_LISTA_FONDO, fg=COLOR_TEXTO_GENERAL, selectbackground=COLOR_BOTON
    )
